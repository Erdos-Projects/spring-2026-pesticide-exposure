# Datasheet: Joint County-Level Dataset (Pesticide + PLACES + Cropland + Census)

*Following Gebru et al. (2018) "Datasheets for Datasets" — combined/summary datasheet*

This document describes the **merged** dataset used for modeling. For source-level details, see the individual datasheets: [USGS Pesticide](datasheet_usgs_pesticide.md), [CDC PLACES](datasheet_cdc_places.md), [USDA Cropland](datasheet_usda_cropland.md), [Census ACS](datasheet_census_acs.md).
...
One row per county. Counties are identified by 5-digit FIPS (state + county code).

**How many instances?**  
~3,066 counties (all counties with pesticide data). The pesticide dataset is the base; other sources are left-joined, so some counties may have missing values for census, cropland, or PLACES.

**What data does each instance consist of?**  
- **Pesticide** (from USGS): `pesticide_total_kg`, `pesticide_respiratory_kg`, chemical class totals (anilide, carbamate, chlorophenoxy, organophosphate, pyrethroid, triazine, etc.), 435+ compound-level columns  
- **Demographics** (from Census ACS): `NAME`, `population`, `median_age`, `median_income`, `pct_white`, `pct_black`, `pct_asian`, `pct_hispanic`  
- **Cropland** (from USDA CDL): `cropland_acres`, `total_area_acres`, `pct_cropland`, `corn_acres`, `soybean_acres`, `cotton_acres`, `wheat_acres`, `hay_acres`, `fruit_veg_acres`, `rice_acres`, `sorghum_acres`, `cropland_diversity`  
- **Health outcomes** (from CDC PLACES): `CASTHMA`, `COPD`, `CSMOKING`, `OBESITY`, `DIABETES` (prevalence %)
...
**Merge order:**  
1. **Base:** USGS Pesticide (county-level, aggregated 2016–2019)  
2. **Left join** Census ACS  
3. **Left join** USDA Cropland  
4. **Left join** CDC PLACES  

**Rationale:** Pesticide is the primary exposure of interest; we retain all pesticide counties. Counties missing from other sources receive NaN for those features.
...
**File:** `data/joint_county_2016_2019.csv`  
**Format:** CSV  
